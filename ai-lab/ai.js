(() => {
  ...
  function startGeneration(){worlds=population.map((brain,i)=>new World(brain,82+generation*997,i));generationHighScore=0;if(!holdPilot)displayPilotIndex=0;displayPilotIndex=clamp(displayPilotIndex,0,POPULATION-1);accumulator=0;updatePilotControls();renderGenerationLog();}
  function finishGeneration(){
    syncScores();const scoreLeader=worlds.reduce((best,w)=>w.score>best.score?w:best,worlds[0]);const longest=Math.max(...worlds.map(w=>w.steps))/SIM_HZ;generationHistory.push({generation,pilot:scoreLeader.index+1,score:generationHighScore,hits:scoreLeader.hits,survival:Number(longest.toFixed(1))});worlds.sort((a,b)=>b.fitness-a.fitness);const champion=worlds[0];if(champion.fitness>bestFitness){bestFitness=champion.fitness;bestBrain=new Brain(rng,champion.brain.toJSON());}
    const elites=worlds.slice(0,ELITES).map(w=>w.brain);population=elites.map(b=>new Brain(rng,b.toJSON()));while(population.length<POPULATION){const parent=elites[Math.floor(rng.next()*elites.length)];population.push(parent.child(rng,.1+Math.min(.12,generation*.001),.24));}generation++;saveProgress();startGeneration();
  }
  ...
  function selectPilot(delta){displayPilotIndex=(displayPilotIndex+delta+POPULATION)%POPULATION;holdPilot=true;updatePilotControls();}
  function renderGenerationLog(){const log=document.getElementById("generationLog"),status=document.getElementById("generationLogStatus");status.textContent=`GEN ${String(generation).padStart(3,"0")} RUNNING`;log.replaceChildren();if(!generationHistory.length){const empty=document.createElement("p");empty.className="generation-empty";empty.textContent="The first record will appear when this generation finishes.";log.append(empty);return;}for(const record of [...generationHistory].reverse()){const row=document.createElement("article");row.className="generation-entry";const gen=document.createElement("strong");gen.textContent=`GEN ${String(record.generation).padStart(3,"0")}`;row.append(gen);for(const [label,value,className] of [["Top pilot",String(record.pilot).padStart(2,"0"),""],["High score",String(record.score).padStart(5,"0"),"record-score"],["Asteroids hit",String(record.hits).padStart(3,"0"),""],["Longest run",`${Number(record.survival).toFixed(1)} SEC`,""]]){const cell=document.createElement("div"),name=document.createElement("span"),data=document.createElement("b");cell.className=className;name.textContent=label;data.textContent=value;cell.append(name,data);row.append(cell);}log.append(row);}}
  function updateUI(w){ui.generationValue.textContent=String(generation).padStart(3,"0");ui.pilotValue.textContent=`${String(w.index+1).padStart(2,"0")} / ${POPULATION}`;ui.scoreValue.textContent=String(w.score).padStart(5,"0");ui.generationScoreValue.textContent=String(generationHighScore).padStart(5,"0");ui.bestScoreValue.textContent=String(bestScore).padStart(5,"0");ui.survivalValue.textContent=`${(w.steps/SIM_HZ).toFixed(1).padStart(4,"0")} SEC`;ui.hitsValue.textContent=String(w.hits).padStart(3,"0");ui.decisionValue.textContent=w.dead?"PILOT LOST":w.decision;ui.actionDetail.textContent=w.dead?"Select another pilot or release hold":w.decision==="OBSERVING"?"Scanning nearest threat":"Neural policy output";ui.pilotNumber.textContent=String(w.index+1).padStart(2,"0");ui.screenScore.textContent=`SCORE ${String(w.score).padStart(5,"0")}`;const pct=clamp((w.fitness/Math.max(1,bestFitness))*100,4,100);ui.fitnessBar.style.width=`${Number.isFinite(pct)?pct:4}%`;ui.fitnessText.textContent=bestFitness>0?`Best fitness ${bestFitness.toFixed(1)}`:"Establishing baseline…";ui.modeValue.textContent=paused?"PAUSED":"TRAINING";ui.signalText.textContent=paused?"SIGNAL HOLD":"SIGNAL ACTIVE";}
  function loop(now){
    ...
    syncScores();let visible=worlds[displayPilotIndex];
    if(!holdPilot&&(!visible||visible.dead)){
