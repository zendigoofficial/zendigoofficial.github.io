(() => {
  ...
  const SIM_HZ = 30, TICK_MS = 1000 / SIM_HZ, FITNESS_VERSION = 2;
  ...
  function loop(now){
    ...
    if(visible){render(visible);updateUI(visible);}if(now-lastAutosaveAt>=5000){lastAutosaveAt=now;saveProgress(true);}requestAnimationFrame(loop);
  }
  ...
  addEventListener("pagehide",()=>saveProgress(true));document.addEventListener("visibilitychange",()=>{if(document.hidden)saveProgress(true);});
  if(archiveReset)saveProgress();if(!restoreActiveRun(savedActiveRun))seedPopulation();requestAnimationFrame(loop);
})();
