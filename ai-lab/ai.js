(() => {
  ...
  const ui = Object.fromEntries(["generationValue","pilotValue","remainingValue","scoreValue","generationScoreValue","bestScoreValue","survivalValue","hitsValue","fitnessBar","fitnessText","decisionValue","actionDetail","modeValue","signalText","pilotNumber","screenScore"].map(id => [id, document.getElementById(id)]));
  ...
  let generation=1,holdPilot=false,generationHighScore=0,bestScore=0,bestFitness=-Infinity,bestBrain=null,generationHistory=[],population=[],worlds=[],last=performance.now(),accumulator=0,displayPilotIndex=0,displayDeathAt=0,generationRecap=null,recapUntil=0;
  ...
  function finishGeneration(now){
    syncScores();const completedGeneration=generation,scoreLeader=worlds.reduce((best,w)=>w.score>best.score?w:best,worlds[0]),longest=Math.max(...worlds.map(w=>w.steps))/SIM_HZ,totalHits=worlds.reduce((total,w)=>total+w.hits,0),pilots=worlds.map(w=>({pilot:w.index+1,score:w.score,hits:w.hits,survival:Number((w.steps/SIM_HZ).toFixed(1))})).sort((a,b)=>b.score-a.score||b.hits-a.hits||b.survival-a.survival).map((pilot,index)=>({...pilot,rank:index+1}));const record={generation:completedGeneration,pilot:scoreLeader.index+1,score:generationHighScore,hits:scoreLeader.hits,totalHits,survival:Number(longest.toFixed(1)),pilots};generationHistory.push(record);worlds.sort((a,b)=>b.fitness-a.fitness);const champion=worlds[0];if(champion.fitness>bestFitness){bestFitness=champion.fitness;bestBrain=new Brain(rng,champion.brain.toJSON());}
  ...
  function renderRecap(record){ctx.clearRect(0,0,W,H);ctx.fillStyle="#010504";ctx.fillRect(0,0,W,H);ctx.textAlign="center";ctx.fillStyle="#76f6cd";ctx.shadowColor="#39c799";ctx.shadowBlur=10;ctx.font="900 40px monospace";ctx.fillText(`GENERATION ${String(record.generation).padStart(3,"0")} COMPLETE`,W/2,150);ctx.shadowBlur=0;ctx.fillStyle="#f0e7dc";ctx.font="700 17px monospace";ctx.fillText(`TOP PILOT ${String(record.pilot).padStart(2,"0")}  //  HIGH SCORE ${String(record.score).padStart(5,"0")}`,W/2,230);ctx.fillStyle="#b3ada8";ctx.font="700 14px monospace";ctx.fillText(`TOTAL HITS ${String(record.totalHits).padStart(3,"0")}  //  LONGEST RUN ${Number(record.survival).toFixed(1)} SEC`,W/2,276);ctx.fillStyle="#ec1932";ctx.font="700 12px monospace";ctx.fillText("NEXT GENERATION PREPARING",W/2,350);ctx.textAlign="left";}
