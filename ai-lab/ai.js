(() => {
  ...
  const MAX_STEPS = 1800, STORAGE_KEY = "zendigo-pixel-asteroids-brain-v1";
  ...
  class World {
    ...
    reset(){
      this.ship={x:W/2,y:H/2,vx:0,vy:0,a:-Math.PI/2,r:11,lives:3,inv:100};
    ...
    fire(){if(this.cooldown>0||this.bullets.length>6)return;this.cooldown=11;this.shots++;this.bullets.push({x:this.ship.x+Math.cos(this.ship.a)*16,y:this.ship.y+Math.sin(this.ship.a)*16,vx:this.ship.vx+Math.cos(this.ship.a)*8,vy:this.ship.vy+Math.sin(this.ship.a)*8,life:72});}
    step(){
  ...
  let generation=1,speed=4,paused=false,bestScore=0,bestFitness=-Infinity,bestBrain=null,population=[],worlds=[],last=performance.now();
  ...
  function startGeneration(){worlds=population.map((brain,i)=>new World(brain,82+generation*997,i));}
  ...
  document.querySelectorAll("[data-speed]").forEach(button=>button.addEventListener("click",()=>{speed=Number(button.dataset.speed);document.querySelectorAll("[data-speed]").forEach(b=>b.classList.toggle("active",b===button));}));
