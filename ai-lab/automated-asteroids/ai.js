(() => {
  ...
  class World {
    ...
    step(){
      ...
      const n=this.nearest(),aimError=Math.abs(angleWrap(Math.atan2(n.dy,n.dx)-this.ship.a)),alignment=Math.max(0,Math.cos(aimError)),motion=clamp(Math.hypot(this.ship.vx,this.ship.vy)/3,0,1),danger=1-clamp(n.d/300,0,1),aimGain=clamp(beforeError-aimError,-.2,.2),spinPenalty=this.turnStreak>45?(this.turnStreak-45)*.00022:0,turnBiasPenalty=Math.max(0,Math.abs(this.turnBias)-.32)*.045,thrustControl=thrust?(danger>.35&&motion<.8?.011:.001):motion>.15?.001:0,stalledPenalty=this.coastStreak>120&&motion<.12?.026:0,overThrustPenalty=this.thrustStreak>90?.012:0;this.fitness+=.018+alignment*.003+Math.max(0,aimGain)*.035+motion*.003+motion*danger*.006+thrustControl-spinPenalty-turnBiasPenalty-stalledPenalty-overThrustPenalty;
