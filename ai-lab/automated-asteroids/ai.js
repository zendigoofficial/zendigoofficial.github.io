(() => {
  ...
  const POPULATION = 24, ELITES = 4, INPUTS = 20, HIDDEN = 20, OUTPUTS = 4;
  const SIM_HZ = 30, TICK_MS = 1000 / SIM_HZ, FITNESS_VERSION = 4;
  const MAX_RUN_SECONDS = 600, MAX_STEPS = MAX_RUN_SECONDS * SIM_HZ;
  const BULLET_SPEED = 8, BULLET_RANGE = BULLET_SPEED * 36;
  const STORAGE_KEY = "zendigo-pixel-asteroids-brain-v5", LEGACY_STORAGE_KEYS = ["zendigo-pixel-asteroids-brain-v4","zendigo-pixel-asteroids-brain-v3","zendigo-pixel-asteroids-brain-v2"];
  ...
  class World {
    ...
    reset(){
      ...
      this.asteroids=[];this.bullets=[];this.score=0;this.hits=0;this.shots=0;this.steps=0;this.fitness=0;this.dead=false;this.decision="OBSERVING";this.lastTurn=0;this.turnStreak=0;this.turnBias=0;this.thrustStreak=0;this.coastStreak=0;this.lastAimError=Math.PI;this.missedShots=0;this.spawnWave(5);
    }
    spawnWave(count){
    ...
    targeting(){
      ...
      if(!n.a)return{...n,rvx:0,rvy:0,leadDx:0,leadDy:0,leadTime:0,leadAngle:this.ship.a};
      const rvx=n.a.vx-this.ship.vx,rvy=n.a.vy-this.ship.vy,c=n.dx*n.dx+n.dy*n.dy,b=2*(n.dx*rvx+n.dy*rvy),a=rvx*rvx+rvy*rvy-BULLET_SPEED*BULLET_SPEED;
      let t=NaN;
      if(Math.abs(a)<1e-6){if(Math.abs(b)>1e-6)t=-c/b;}
      else{const disc=b*b-4*a*c;if(disc>=0){const root=Math.sqrt(disc),t1=(-b-root)/(2*a),t2=(-b+root)/(2*a),valid=[t1,t2].filter(v=>v>0);if(valid.length)t=Math.min(...valid);}}
      if(!Number.isFinite(t)||t<=0)t=Math.hypot(n.dx,n.dy)/BULLET_SPEED;
      t=clamp(t,0,36);
      const leadDx=n.dx+rvx*t,leadDy=n.dy+rvy*t;
      return{...n,rvx,rvy,leadDx,leadDy,leadTime:t,leadAngle:Math.atan2(leadDy,leadDx)};
    }
    collisionThreat(){
      let best={a:null,danger:0,time:90,miss:W,dx:0,dy:0};
      for(const a of this.asteroids){
        const dx=torusDelta(this.ship.x,a.x,W),dy=torusDelta(this.ship.y,a.y,H),rvx=a.vx-this.ship.vx,rvy=a.vy-this.ship.vy,v2=rvx*rvx+rvy*rvy;
        const time=v2>.0001?clamp(-(dx*rvx+dy*rvy)/v2,0,90):90,miss=Math.hypot(dx+rvx*time,dy+rvy*time),clearance=a.r+this.ship.r+34;
        const approach=clamp(1-time/90,0,1),path=clamp(1-miss/clearance,0,1),proximity=clamp(1-(Math.hypot(dx,dy)-a.r)/220,0,1),danger=Math.max(path*approach,proximity*.58);
        if(danger>best.danger)best={a,danger,time,miss,dx,dy};
      }
      return best;
    }
    inputs(){
      const n=this.targeting(),threat=this.collisionThreat(),diff=angleWrap(n.leadAngle-this.ship.a),speed=Math.hypot(this.ship.vx,this.ship.vy),forward=(this.ship.vx*Math.cos(this.ship.a)+this.ship.vy*Math.sin(this.ship.a))/6,lateral=(-this.ship.vx*Math.sin(this.ship.a)+this.ship.vy*Math.cos(this.ship.a))/6,closing=n.a?(n.rvx*n.dx+n.rvy*n.dy)/Math.max(1,Math.hypot(n.dx,n.dy))/6:0,escape=threat.a?angleWrap(Math.atan2(-threat.dy,-threat.dx)-this.ship.a):0;
      return new Float32Array([Math.sin(diff),Math.cos(diff),clamp(n.d/560,0,1),clamp(n.leadDx/W,-1,1),clamp(n.leadDy/H,-1,1),clamp(n.rvx/6,-1,1),clamp(n.rvy/6,-1,1),clamp(speed/6,0,1),forward,lateral,clamp(closing,-1,1),clamp(threat.time/90,0,1),clamp(threat.miss/180,0,1),Math.sin(escape),Math.cos(escape),threat.danger,clamp(this.cooldown/12,0,1),clamp(this.asteroids.length/14,0,1),clamp(this.turnBias,-1,1),1]);
    }
    fire(){if(this.cooldown>0||this.bullets.length>6)return false;this.cooldown=6;this.shots++;this.bullets.push({x:this.ship.x+Math.cos(this.ship.a)*16,y:this.ship.y+Math.sin(this.ship.a)*16,vx:this.ship.vx+Math.cos(this.ship.a)*BULLET_SPEED,vy:this.ship.vy+Math.sin(this.ship.a)*BULLET_SPEED,life:36});return true;}
    step(){
      if(this.dead)return;this.steps++;this.cooldown=Math.max(0,(this.cooldown||0)-1);this.ship.inv=Math.max(0,this.ship.inv-1);
      const before=this.targeting(),beforeError=Math.abs(angleWrap(before.leadAngle-this.ship.a)),out=this.brain.think(this.inputs());let turn=0;const left=out[0]>.18,right=out[1]>.18;if(left!==right)turn=left?-1:1;else if(left&&right)turn=out[0]>out[1]?-1:1;const thrust=out[2]>.16,fire=out[3]>.08;
      if(turn&&turn===this.lastTurn)this.turnStreak++;else this.turnStreak=turn?1:0;this.lastTurn=turn;this.turnBias=this.turnBias*.985+turn*.015;this.thrustStreak=thrust?this.thrustStreak+1:0;this.coastStreak=thrust?0:this.coastStreak+1;
      this.ship.a+=turn*.075;if(thrust){this.ship.vx+=Math.cos(this.ship.a)*.105;this.ship.vy+=Math.sin(this.ship.a)*.105;}const speed=Math.hypot(this.ship.vx,this.ship.vy);if(speed>6){this.ship.vx*=6/speed;this.ship.vy*=6/speed;}this.ship.vx*=.997;this.ship.vy*=.997;this.ship.x=wrap(this.ship.x+this.ship.vx,W);this.ship.y=wrap(this.ship.y+this.ship.vy,H);const shotFired=fire?this.fire():false;
      ...
      for(const b of this.bullets){b.x=wrap(b.x+b.vx,W);b.y=wrap(b.y+b.vy,H);b.life--;if(b.life===0){this.missedShots++;this.fitness-=.8;}}this.bullets=this.bullets.filter(b=>b.life>0);
      for(const a of this.asteroids){a.x=wrap(a.x+a.vx,W);a.y=wrap(a.y+a.vy,H);a.rot+=a.spin;}
      outer:for(let bi=this.bullets.length-1;bi>=0;bi--){const b=this.bullets[bi];for(let ai=this.asteroids.length-1;ai>=0;ai--){const a=this.asteroids[ai],dx=torusDelta(b.x,a.x,W),dy=torusDelta(b.y,a.y,H);if(Math.hypot(dx,dy)<a.r){this.bullets.splice(bi,1);this.asteroids.splice(ai,1);this.hits++;const points=a.r>31?20:a.r>18?50:100;this.score+=points;this.fitness+=points*2+35;if(a.r>18){for(let k=0;k<2;k++){const ang=this.rng.range(0,TAU);this.asteroids.push({...a,r:a.r*.58,vx:a.vx+Math.cos(ang)*1.25,vy:a.vy+Math.sin(ang)*1.25,shape:Array.from({length:9},()=>this.rng.range(.72,1.14))});}}break outer;}}}
      if(this.ship.inv<=0){for(const a of this.asteroids){const dx=torusDelta(this.ship.x,a.x,W),dy=torusDelta(this.ship.y,a.y,H);if(Math.hypot(dx,dy)<a.r+this.ship.r){this.ship.lives--;this.fitness-=100;if(this.ship.lives<=0){this.dead=true;this.decision="GAME OVER";}break;}}}
      const n=this.targeting(),threat=this.collisionThreat(),aimError=Math.abs(angleWrap(n.leadAngle-this.ship.a)),alignment=Math.max(0,Math.cos(aimError)),motion=clamp(Math.hypot(this.ship.vx,this.ship.vy)/3,0,1),danger=threat.danger,aimGain=clamp(beforeError-aimError,-.25,.25),escapeAngle=threat.a?angleWrap(Math.atan2(-threat.dy,-threat.dx)-this.ship.a):0,escapeAlignment=Math.cos(escapeAngle),shipSpeed=Math.max(.01,Math.hypot(this.ship.vx,this.ship.vy)),escapeVelocity=threat.a?clamp((this.ship.vx*-threat.dx+this.ship.vy*-threat.dy)/(shipSpeed*Math.max(1,Math.hypot(threat.dx,threat.dy))),-1,1):0,spinPenalty=this.turnStreak>30?Math.min(.14,(this.turnStreak-30)*.0018):0,turnBiasPenalty=Math.max(0,Math.abs(this.turnBias)-.22)*.08,thrustControl=thrust?(danger*(Math.max(0,escapeAlignment)*.04-Math.max(0,-escapeAlignment)*.055)+(shipSpeed<.8?.004:0)):0,movementCredit=danger*Math.max(0,escapeVelocity)*.014,stalledPenalty=this.coastStreak>90&&motion<.12?.02:0,overThrustPenalty=this.thrustStreak>75?.012:0,badTurnPenalty=turn&&aimGain<-.015&&danger<.55?.012:0,shotControl=shotFired?(beforeError<.18&&before.d<BULLET_RANGE?0.07:-.14-Math.max(0,beforeError-.35)*.07):0;this.fitness+=.006+alignment*.004+Math.max(0,aimGain)*.09+movementCredit+thrustControl+shotControl-spinPenalty-turnBiasPenalty-stalledPenalty-overThrustPenalty-badTurnPenalty;
  ...
  try{let saved=JSON.parse(localStorage.getItem(STORAGE_KEY)||"null");if(!saved){for(const key of LEGACY_STORAGE_KEYS){saved=JSON.parse(localStorage.getItem(key)||"null");if(saved){archiveReset=true;break;}}}if(saved){const compatible=saved.fitnessVersion===FITNESS_VERSION;bestScore=saved.bestScore||0;bestScorePilot=saved.bestScorePilot||0;bestScoreGeneration=saved.bestScoreGeneration||0;bestFitness=compatible&&Number.isFinite(saved.bestFitness)?saved.bestFitness:-Infinity;generation=saved.generation||1;generationHistory=Array.isArray(saved.history)?saved.history:[];savedActiveRun=compatible?saved.activeRun||null:null;savedAt=compatible&&Number.isFinite(Number(saved.savedAt))?Number(saved.savedAt):Date.now();if(!compatible)archiveReset=true;if(!bestScorePilot&&generationHistory.length){const record=generationHistory.reduce((best,item)=>item.score>best.score?item:best,generationHistory[0]);bestScorePilot=record.pilot||0;bestScoreGeneration=record.generation||0;}if(compatible&&saved.brain)bestBrain=new Brain(rng,saved.brain);}}catch{}
  function snapshotActiveRun(){if(!worlds.length||generationRecap)return null;return{generation,generationHighScore,generationHighPilot,displayPilotIndex,holdPilot,worlds:worlds.map(w=>({brain:w.brain.toJSON(),rngState:w.rng.s,index:w.index,ship:{...w.ship},asteroids:w.asteroids.map(a=>({...a,shape:[...a.shape]})),bullets:w.bullets.map(b=>({...b})),score:w.score,hits:w.hits,shots:w.shots,steps:w.steps,fitness:w.fitness,dead:w.dead,decision:w.decision,cooldown:w.cooldown||0,lastTurn:w.lastTurn||0,turnStreak:w.turnStreak||0,turnBias:Number(w.turnBias)||0,thrustStreak:w.thrustStreak||0,coastStreak:w.coastStreak||0,lastAimError:Number(w.lastAimError)||Math.PI,missedShots:w.missedShots||0}))};}
